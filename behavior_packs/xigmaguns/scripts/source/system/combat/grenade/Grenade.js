...
import { FlashbangHandler, SmokeGrenadeHandler } from "./GrenadeHandlers";
...
export class Grenade {
    ...
    setHandler(projectile) {
        const family = projectile.getComponent('type_family');
        if (family !== undefined) {
            if (family.hasTypeFamily('smoke_grenade'))
                return new SmokeGrenadeHandler(projectile);
            if (family.hasTypeFamily('flashbang'))
                return new FlashbangHandler(projectile);
        }
    ...
    projectileRebound() {
        ...
        const hitBlockRebound = async (ev) => {
            if (ev.projectile.id !== this.handler.projectile.id)
                return;
            const projectile = this.handler.projectile;
            const hitBlockInfo = ev.getBlockHit();
            if (!bounces.has(projectile))
                bounces.set(projectile, 1);
            ...
            entity.getComponent('projectile').shoot(Vector.mul(Vector.mul(ev.hitVector, Math.pow(decreaseValue, count)), mirrored[hitBlockInfo.face]));
        };
    ...
    static throwing(owner, grenadeItem) {
        const triggers = {
        ...
        for (const [grenadeType, eventName] of Object.entries(triggers)) {
            if (grenadeItem.hasTag(grenadeType))
                owner.triggerEvent(eventName);
        }
    }
}
